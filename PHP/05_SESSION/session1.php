...
echo '<br> 2 - La session après suppression de mdp : <br>';
var_dump($_SESSION);
...
var_dump($_SESSION);  // nous voyons encore le contenu de la session ici. En effet, il faut savoir que session_destroy() est d'abord lu par l'interpréteur, puis exécuter réellement qu'à la toute fin du script : pour le voir, vérifier le dossier xampp/tmp/ ou la session n'existe plus.

// Les session on l'avantage d'être disponibles partout sur le site, et donc tous les scripts (voir session2.php).