...
// PDO pour PHP Data Objects est une extension de PHP qui définit une interface pour accèder à une base de données depuis OHP (via du SQL).
...
// $pdo est un objet issu de la classe prédéfinie BDO. Il représente la connexion à la BDD.
...
// exec est utilisé pour la formulation de requête ne retournant pas de jeu de résultats : INSERT, DELETE; UPDATE.
...
echo 'Dernier id généré après la requête : ' . $pdo->lastInsertId();  // méthode qui permet de récupérer depuis la BDD le dernier id inséré par la requête précèdente
//--------
...
// Au contraire de exec(), query() est utilisé pour les requêtes qui retournent un ou plusieurs résultats provenant de la BDD : SELECT. Notez que query() peut être aussi utilisé avec INSERT, UPDATE ou DELETE.
...
$result = $pdo->query("SELECT * FROM employes WHERE prenom = 'daniel'");
$employe = $result->fetch(PDO::FETCH_OBJ);  // transforme en un objet avec les noms des champs de la table comme propriètés de cet objet
debug($employe);
